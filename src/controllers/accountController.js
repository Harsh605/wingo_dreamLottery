...
const verifyCode = async (req, res) => {
  ...
  if (rows.length == 0) {
    await request(
      `https://www.fast2sms.com/dev/bulkV2?authorization=YhyVlPwFBAMfXmg57e40JQ1vEKL6jNpZHbxuWzoc2niUrI98TGCs1apeNLz04lJmocbKMxRrg8Ekdi7V&variables_values=${otp}&route=otp&numbers=${phone}`,
      async (error, response, body) => {
        ...
        if (data.message == "SMS sent successfully.") {
          ...
          return res.status(200).json({
            ...
            status: true,
            timeStamp: now, // change timeNow to now
            timeEnd: timeEnd,
          });
        }
      }
