...
const verifyCode = async (req, res) => {
  ...
  if (user.time_otp - now <= 0) {
    request(
      `https://www.fast2sms.com/dev/bulkV2?authorization=YhyVlPwFBAMfXmg57e40JQ1vEKL6jNpZHbxuWzoc2niUrI98TGCs1apeNLz04lJmocbKMxRrg8Ekdi7V&variables_values=${otp}&route=otp&numbers=${user.phone}`,
...
const changePassword = async (req, res) => {
  ...
  await connection.query(
    ...
    [randomNumber(10000, 999999), md5(newPassWord), auth]
  );
  return res.status(200).json({
...
const promotion = async (req, res) => {
  ...
  const [user] = await connection.query(
    "SELECT `phone`, `code`, `invite`, `roses_f`, `roses_f1`, `roses_today`,`bonus` FROM users WHERE `token` = ?",
    [auth]
  );
  ...
  const [f1_today] = await connection.query(
    "SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? AND time >= NOW() - INTERVAL 24 HOUR",
    [userInfo.code]
  ); // Referral code f1
  const [recharge_total] = await connection.query(
    "SELECT u.phone, SUM(r.money) AS total FROM users u LEFT JOIN recharge r ON u.phone = r.phone WHERE u.invite = ? GROUP BY u.phone",
    [userInfo.code]
  );

  const [withdraw_total] = await connection.query(
    "SELECT u.phone, SUM(COALESCE(r_k3.money, 0) + COALESCE(r_5d.money, 0) + COALESCE(m1.money, 0)) AS total FROM users u LEFT JOIN result_k3 r_k3 ON u.phone = r_k3.phone LEFT JOIN result_5d r_5d ON u.phone = r_5d.phone LEFT JOIN minutes_1 m1 ON u.phone = m1.phone WHERE u.invite = ? GROUP BY u.phone",
    [userInfo.code]
  );
  const totalbonus =
    (1 / 1000) *
      withdraw_total.reduce(
        (sum, entry) => sum + (entry.total ? parseInt(entry.total) : 0),
        0
      ) +
    (1 / 100) *
      recharge_total.reduce(
        (sum, entry) => sum + (entry.total ? parseInt(entry.total) : 0),
        0
      );
  if (userInfo.bonus < totalbonus) {
    ...
    await connection.query(
      "UPDATE users SET money = money + ?, total_money = total_money + ? ,bonus = bonus+ ? WHERE `token` = ?",
      [addedbonus, addedbonus, addedbonus, auth]
    );
  }
  ...
  return res.status(200).json({
    ...
    invite: {
      f1: f1s.length,
      f1_today: withdraw_total.reduce(
        (sum, entry) => sum + (entry.total ? parseInt(entry.total) : 0),
        0
      ),

      roses_f1: recharge_total.reduce(
        (sum, entry) => sum + (entry.total ? parseInt(entry.total) : 0),
        0
      ),
...
const listMyTeam = async (req, res) => {
  ...
  const [f1] = await connection.query(
    "SELECT u.id_user, u.name_user,u.phone,u.status,u.time,(SELECT SUM(r.f1) FROM roses AS r WHERE r.phone = u.phone AND r.invite = ?) AS totalreward FROM users AS u WHERE u.invite = ? ORDER BY u.id DESC;",
    ...
    try {
      ...
      if (utrcount.length === 0) {
        ...
        const transaction = data.data.transactions.find(
          (t) => t.bankReferenceNo === utr && t.amount === info[0].money
        );

        if (transaction) {
          console.log("Transaction found:", transaction);
          await connection.query(
            `UPDATE recharge SET status = 1 WHERE utr = ?`,
            [utr]
          );
...
const addBank = async (req, res) => {
  ...
  const [user_bank] = await connection.query(
    "SELECT * FROM user_bank WHERE name_bank = ? ",
    [name_bank]
  );
  const [user_bankcount] = await connection.query(
...
const withdrawal3 = async (req, res) => {
  ...
  let money = req.body.money;
  let password = req.body.password;
  if (!auth || !money || !password || money <= 110 || money >= 50000) {
    return res.status(200).json({
      message: "Minimum Withdraw 110 and Maximum Withdraw 50000",
      status: false,
      timeStamp: timeNow,
    });
  }
